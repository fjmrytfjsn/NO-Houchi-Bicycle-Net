...
- 持ち主が仮解除を申請できる
- 一定時間経過後に本解除できる
- 本解除後、即時移動の心理的トリガーとして、15分限定のランダム額クーポンのモック機能（SMS認証・ルーレット）を提供する
...
- 本解除完了後、SMS認証による回数制限（モック）、ルーレットによる金額決定、および「15分で消滅する」クーポン画面を提示し、即時移動を促す心理的トリガーを提供する
...
sequenceDiagram
  ...
  API->>DB: 通報順位を判定し、該当サポーターへポイント付与（※1）
```

※1: 同じ自転車（マーカー）に複数人が通報していた場合、全員にポイントが付与されるが、通報した順番（早い順）によってもらえるポイント量が変動する。
...
sequenceDiagram
  ...
  API->>DB: 通報順位を判定し、該当サポーターへポイント付与
```
...
通報者（サポーター）へのインセンティブとして、通報した案件が「解決（持ち主による解除）」または「撤去（回収完了）」されたタイミングでポイントが付与される。

- **複数通報の扱い**: 同じ自転車（同一マーカー）に対して複数のサポーターが通報を行った場合、通報者全員がポイント獲得の対象となる。
- **ポイントの傾斜配分**: 通報した順（通報日時の早い順）で付与されるポイント量が変動する。一番早く通報したサポーターが最も多くのポイントを獲得し、以降は段階的に減少する。これにより「早く見つけて通報する」モチベーションを高めつつ、後から通報したサポーターの徒労感も防ぐ。

### 7.5 将来拡張: 地域協賛・クーポン連携

地域協賛型では、サポーターの参加や持ち主の自発的な移動を促すため、クーポン等を提供する。持ち主向けには「単なるご褒美」とならないよう、本解除後にSMS認証（受取制限）や15分間の超短期タイマーを設け、損失回避の心理を利用して即時移動を促すトリガーとして設計する。試作品ではこれをフロントエンドのモックとして実装し、バックエンドの実発行管理、店舗精算、外部システム連携等は将来構想とする。
...
- v1.7: サポーター向けのポイント付与フロー（解除・撤去時の順位連動ポイント付与）をデータフローに追加
- v1.6: クーポンの本来の目的（即時移動のトリガー）に合わせ、SMS認証・ルーレット・15分タイマーのUX仕様を反映
